      ...
      function showUserCreateBox() {
        Swal.fire({
          ...
          preConfirm: () => {
            ...
            const stime = Swal.getPopup().querySelector('#stime').value
            const etime = Swal.getPopup().querySelector('#etime').value
            const barber = Swal.getPopup().querySelector('#barber').value
            const style = Swal.getPopup().querySelector('#style').value
            const product = Swal.getPopup().querySelector('#product').value
            const membership = Swal.getPopup().querySelector('#newmembership').value
            if(stime.match(/([01]?[0-9]|2[0-3]):[0-5][0-9]/) == null || etime.match(/([01]?[0-9]|2[0-3]):[0-5][0-9]/) == null){
              Swal.showValidationMessage(`Valid time (00:00)`)
            }
            if(newdate.match(/^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/) == null){
              Swal.showValidationMessage(`Valid date (YYYY-MM-DD)`)
            }
            if(stime == etime){
              Swal.showValidationMessage(`Please enter different start and end times`)
            }
            if(membership.match(/gift card/i) == null && membership.match(/free voucher/i) == null && membership.match(/paid service/i) == null && membership.match(/annual/i) == null){
              Swal.showValidationMessage(`Valid memberships: annual, gift card, paid service, free voucher`)
            }
            if (!name || !newdate || !stime || !etime || !barber || !style || !product || !membership) {
              Swal.showValidationMessage(`Please enter all the fields to create appointment`)
            }
      ...
      function showUserEditBox(eventid) {
        ...
        Swal.fire({
          ...
          cancelButtonAriaLabel: 'Cancel',
          preConfirm: () => {
            const name = Swal.getPopup().querySelector('#namee').value
            const newdate = Swal.getPopup().querySelector('#newdatee').value
            const stime = Swal.getPopup().querySelector('#stimee').value
            const etime = Swal.getPopup().querySelector('#etimee').value
            const barber = Swal.getPopup().querySelector('#barbere').value
            const style = Swal.getPopup().querySelector('#stylee').value
            const product = Swal.getPopup().querySelector('#producte').value
            const membership = Swal.getPopup().querySelector('#newmembershipe').value
            if((stime.match(/([01]?[0-9]|2[0-3]):[0-5][0-9]/) == null || etime.match(/([01]?[0-9]|2[0-3]):[0-5][0-9]/) == null) && (stime.trim() != '' || etime.trim() != '')){
              Swal.showValidationMessage(`Valid time (00:00)`)
            }
            if(newdate.match(/^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/) == null && newdate.trim() != ''){
              Swal.showValidationMessage(`Valid date (YYYY-MM-DD)`)
            }
            if(stime == etime && stime.trim() != '' && etime.trim() != ''){
              Swal.showValidationMessage(`Please enter different start and end times`)
            }
            if(membership.trim() != '' && membership.match(/gift card/i) == null && membership.match(/free voucher/i) == null && membership.match(/paid service/i) == null && membership.match(/annual/i) == null){
              Swal.showValidationMessage(`Valid memberships: annual, gift card, paid service, free voucher`)
            }
            if (!name && !newdate && !stime && !etime && !barber && !style && !product && !membership) {
              Swal.showValidationMessage(`Please enter a field to edit the appointment`)
            }
      ...
      function userEdit(eventid) {
        ...
        if (name.trim() == ''){
          name = eventid.split(",")[1].trim();
        }
        if (newdate.trim() == ''){
          newdate = eventid.split(",")[2].trim();
        }
        if (stime.trim() == ''){
          tempstime = eventid.split(",")[3];
          stime = tempstime.split(" ")[0].trim();
        }
        if (etime.trim() == ''){
          tempetime = eventid.split(",")[3];
          etime = tempetime.split(" ")[2].trim();
        }
        if (barber.trim() == ''){
        ...
        if (product.trim() == ''){
          product = eventid.split(",")[6].trim();
        }
        if (newmembership.trim() == ''){
          newmembership = eventid.split(",")[7].trim();
        }
      ...
      // Table Loop
       ...
       async function listUpcomingEvents() {
        ...
        for (row = 0; row < output.split("\n").length - 1; row++) {
          ...
          fullevent = eventids[count - 1] + "," + String(output.split(")")[i1].trim()) + "," + dates[count - 1] + "," + startingtime[count - 1] + " To " + endingtime[count - 1] + ","+barber[count - 1] + ","+style[count - 1] + ","+ products[count - 1] + "," + membershiptype[count - 1];
          console.log(fullevent);
          td8.innerHTML = "<button type='button' class='btn btn-outline-secondary' onclick='editEvent(\"" + fullevent + "\");'>Edit</button><button type='button' class='btn btn-outline-danger' onclick='deleteEvent(\""+ currenteventid+ "\");'>Del</button>";
